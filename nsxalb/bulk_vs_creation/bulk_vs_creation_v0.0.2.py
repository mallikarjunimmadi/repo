...
def network_uuid_from_ref(name_or_ref):
    value = (name_or_ref or "").strip()
    if "/api/network/" not in value:
        return value if value.startswith("network-") else None
    tail = value.split("/api/network/", 1)[1]
    return tail.split("#", 1)[0].split("?", 1)[0].strip("/") or None

def network_name_from_ref(name_or_ref):
    value = (name_or_ref or "").strip()
    if "#" in value:
        return value.rsplit("#", 1)[1]
    return value

def network_ref_for_payload(name_or_ref, network):
    value = (name_or_ref or "").strip()
    if "/api/network/" in value:
        return value
    if network.get("url"):
        return network["url"]
    if network.get("uuid"):
        return f"/api/network/{network['uuid']}"
    return None

def extract_ipv4_subnet(network):
    for key in ("configured_subnets", "subnet_runtime"):
        for item in network.get(key) or []:
            prefix = item.get("prefix") or item.get("subnet") or item
            ip_addr = prefix.get("ip_addr") or prefix.get("ip") or {}
            mask = prefix.get("mask")
            addr = ip_addr.get("addr")
            ip_type = ip_addr.get("type") or "V4"
            if addr and mask is not None and ip_type == "V4":
                return {"addr": addr, "mask": mask}
    return None

def find_cached_network(nets, cloud_uuid, name_or_ref):
    value = (name_or_ref or "").strip()
    uuid = network_uuid_from_ref(value)
    name = network_name_from_ref(value)
    for n in nets.get(cloud_uuid, []):
        if uuid and uuid == n.get("uuid"):
            return n
        if name in (n.get("name"), n.get("uuid"), n.get("url")):
            return n
    return None

def fetch_network_by_uuid(api, uuid):
    if not uuid:
        return None
    return api_json(api, f"network/{uuid}") or api_json(api, f"/api/network/{uuid}")

def fetch_network_by_name(api, name, cloud_uuid):
    if not name:
        return None
    data = api_json(api, f"network?name={quote(name)}")
    for n in (data or {}).get("results", []):
        if n.get("name") != name:
            continue
        if cloud_uuid and n.get("cloud_ref", "").split("/")[-1] != cloud_uuid:
            continue
        return n
    return None

def fetch_network_details(api, log, nets, cloud_uuid, cloud_name, name_or_ref, dbg=False):
    value = (name_or_ref or "").strip()
    if not value:
        return None

    cached = find_cached_network(nets, cloud_uuid, value)
    uuid = network_uuid_from_ref(value) or (cached or {}).get("uuid")
    network = fetch_network_by_uuid(api, uuid)

    if not network:
        network = fetch_network_by_name(api, network_name_from_ref(value), cloud_uuid)

    if not network and cached:
        network = cached

    if not network:
        log.warning(f"Network '{value}' not found in cloud {cloud_name}")
        return None

    subnet = extract_ipv4_subnet(network)
    if not subnet:
        log.warning(f"Network '{value}' has no IPv4 configured_subnets/subnet_runtime entry.")
        return None

    network_ref = network_ref_for_payload(value, network)
    if not network_ref:
        log.warning(f"Network '{value}' has no usable network_ref/url.")
        return None

    details = {
        "uuid": network.get("uuid"),
        "name": network.get("name") or network_name_from_ref(value),
        "network_ref": network_ref,
        "subnet_addr": subnet["addr"],
        "subnet_mask": subnet["mask"]
    }
    if dbg:
        log.debug(f"[NETWORK DETAILS] {value} -> {pp(details)}")
    return details
...
def create_pool(api, log, name, members, net_name, cu, cn, dry, dbg, nets, health_monitor, app_prof):
    ...
    if net_name:
        nd=fetch_network_details(api,log,nets,cu,cn,net_name,dbg)
        if nd:
            data["placement_networks"]=[{
                "network_ref":nd["network_ref"],
                "subnet":{
...
def create_or_reuse_vsvip(api, log, name, vs_ip, vip_net, cu, cn, dry, dbg, nets):
    ...
    nd=fetch_network_details(api,log,nets,cu,cn,vip_net,dbg) if vip_net else None
    vip_block={
