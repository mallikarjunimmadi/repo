...
            if nxt.startswith("/"):
                # This assumes the base_url used to log in is correct
                if not base: # Defensive check if base wasn't set from first_url
...
        if datapoints:
            first = datapoints[0]
            # --- FIX: Ensure we only show the value (not timestamp) ---
            # Avi metrics usually return [timestamp, value]
            if isinstance(first, (list, tuple)):
                if len(first) > 1:
                    # Standard [timestamp, value] pair
                    series_map[mname] = first[1] 
                elif len(first) == 1:
                    # Handles unexpected [value] format
                    series_map[mname] = first[0]
                else:
                    series_map[mname] = "N/A"
            else:
                # Handles unexpected scalar/raw number return
...
        # The previous exception was here: fut.result() -> process_vs_row
        # The fix is in process_vs_row now.
...
            # --- The original traceback exception occurred here ---
