...
HIGH_RTT_SEC_THRESHOLD = 0.2
SPIKY_RTT_SPREAD_SEC_THRESHOLD = 0.03
...
class StreamAgg:
    ...
    first_high_rtt_frame: int = 0
    first_high_rtt_rtt_sec: float = 0.0
    first_high_rtt_acks_frame: int = 0
    ...
    ack_rtt_min_epoch: float = 0.0
    ack_rtt_min_frame: int = 0
    ack_rtt_min_acks_frame: int = 0
    ack_rtt_max_sec: float = 0.0
    ack_rtt_max_epoch: float = 0.0
    ack_rtt_max_frame: int = 0
    ack_rtt_max_acks_frame: int = 0
    max_interpacket_gap_sec: float = 0.0


@dataclass
class RunningCsvLogger:
    handle: object
...
def _run_tshark_command(cmd: List[str]) -> subprocess.CompletedProcess:
    """
    Run tshark with a compatibility fallback for builds that reject
    the optional `-o x11.tcp.port:` preference override.
    """
    try:
        return subprocess.run(cmd, capture_output=True, text=True, check=True)
    except subprocess.CalledProcessError as e:
        stderr = e.stderr or ""
        if 'Invalid -o flag "x11.tcp.port:"' in stderr:
            fallback_cmd = [part for part in cmd if part not in ("-o", "x11.tcp.port:")]
...
def open_rtt_running_csv(out_csv: str, utc: bool, input_pcap: str) -> RunningCsvLogger:
    fieldnames = [
        "input_pcap", "time_mode", "protocol", "stream_id", "frame_number", "packet_ts",
        "src_ip", "src_port", "dst_ip", "dst_port",
        "acks_frame",
        "packet_rtt_ms", "running_ack_rtt_samples",
        "running_rtt_min_ms", "running_rtt_max_ms", "running_rtt_spread_ms",
        "high_rtt_threshold_ms", "packet_exceeds_high_rtt", "first_high_rtt_trigger",
    ]
    f = open(out_csv, "w", newline="")
    writer = csv.DictWriter(f, fieldnames=fieldnames)
    writer.writeheader()
    return RunningCsvLogger(handle=f, writer=writer)
...
def ingest_proto(
    ...
    rtt_running_logger: Optional[RunningCsvLogger] = None,
    utc: bool = False,
) -> None:
    ...
    for line in run_tshark(pcap, proto, display_filter=display_filter):
        ...
        if is_tcp:
            ...
            try:
                ...
                ack_rtt_sec = 0.0
            try:
                acked_frame_no = int(acks_frame_s) if acks_frame_s else 0
            except ValueError:
                acked_frame_no = 0
            ...
            if ack_rtt_sec > 0.0:
                first_high_trigger = (
                    ack_rtt_sec >= HIGH_RTT_SEC_THRESHOLD and st.first_high_rtt_epoch == 0.0
                )
                st.ack_rtt_samples += 1
                if st.ack_rtt_min_sec == 0.0 or ack_rtt_sec < st.ack_rtt_min_sec:
                    st.ack_rtt_min_sec = ack_rtt_sec
                    st.ack_rtt_min_epoch = epoch
                    st.ack_rtt_min_frame = frame_no
                    st.ack_rtt_min_acks_frame = acked_frame_no
                if ack_rtt_sec > st.ack_rtt_max_sec:
                    st.ack_rtt_max_sec = ack_rtt_sec
                    st.ack_rtt_max_epoch = epoch
                    st.ack_rtt_max_frame = frame_no
                    st.ack_rtt_max_acks_frame = acked_frame_no
                if ack_rtt_sec >= HIGH_RTT_SEC_THRESHOLD and st.first_high_rtt_epoch == 0.0:
                    st.first_high_rtt_epoch = epoch
                    st.first_high_rtt_frame = frame_no
                    st.first_high_rtt_rtt_sec = ack_rtt_sec
                    st.first_high_rtt_acks_frame = acked_frame_no
                if rtt_running_logger is not None:
                    rtt_running_logger.write_row({
                        "input_pcap": os.path.basename(pcap),
                        "time_mode": "UTC" if utc else "Local",
                        "protocol": st.proto,
                        "stream_id": st.stream_id,
                        "frame_number": frame_no,
                        "packet_ts": epoch_to_iso(epoch, utc),
                        "src_ip": st.src_ip,
                        "src_port": st.src_port,
                        "dst_ip": st.dst_ip,
                        "dst_port": st.dst_port,
                        "acks_frame": acked_frame_no,
                        "packet_rtt_ms": round(ack_rtt_sec * 1000.0, 3),
                        "running_ack_rtt_samples": st.ack_rtt_samples,
                        "running_rtt_min_ms": round(st.ack_rtt_min_sec * 1000.0, 3),
                        "running_rtt_max_ms": round(st.ack_rtt_max_sec * 1000.0, 3),
                        "running_rtt_spread_ms": round(max(0.0, st.ack_rtt_max_sec - st.ack_rtt_min_sec) * 1000.0, 3),
                        "high_rtt_threshold_ms": round(HIGH_RTT_SEC_THRESHOLD * 1000.0, 3),
                        "packet_exceeds_high_rtt": "TRUE" if ack_rtt_sec >= HIGH_RTT_SEC_THRESHOLD else "FALSE",
                        "first_high_rtt_trigger": "TRUE" if first_high_trigger else "FALSE",
                    })
...
def finalize_additional_categories(rows: List[StreamAgg]) -> None:
    for st in rows:
        ...
        st.spiky_rtt = (
            st.ack_rtt_max_sec >= HIGH_RTT_SEC_THRESHOLD and
...
def to_row_dict(st: StreamAgg, utc: bool) -> dict:
    ...
    return {
        ...
        "first_high_rtt_frame": st.first_high_rtt_frame,
        "first_high_rtt_ts": epoch_to_iso(st.first_high_rtt_epoch, utc) if st.first_high_rtt_epoch > 0.0 else "",
        "first_high_rtt_ms": round(st.first_high_rtt_rtt_sec * 1000.0, 3),
        "first_high_rtt_acks_frame": st.first_high_rtt_acks_frame,
        "ack_rtt_min_frame": st.ack_rtt_min_frame,
        "ack_rtt_min_acks_frame": st.ack_rtt_min_acks_frame,
        "ack_rtt_max_frame": st.ack_rtt_max_frame,
        "ack_rtt_max_acks_frame": st.ack_rtt_max_acks_frame,
...
def write_csv(out_csv: str, rows: List[StreamAgg], utc: bool) -> None:
    fieldnames = [
        ...
        "ack_rtt_samples", "ack_rtt_max_ms", "ack_rtt_spread_ms",
        "first_high_rtt_frame", "first_high_rtt_ts", "first_high_rtt_ms", "first_high_rtt_acks_frame",
        "ack_rtt_min_frame", "ack_rtt_min_acks_frame", "ack_rtt_max_frame", "ack_rtt_max_acks_frame", "avg_pps",
        "conn_refused_syn_rst",
        "client_abort_after_synack",
        "conn_failed_ackretrans_rst",
        "has_retransmissions",
...
def _rtt_evidence_rows(rows: List[StreamAgg], utc: bool) -> List[dict]:
    evidence: List[dict] = []
    seen_keys = set()
    ...
    def add_event(
        ...
        if frame_no <= 0 or epoch <= 0.0 or rtt_sec <= 0.0:
            return
        key = (st.proto, st.stream_id, event_type, frame_no)
        if key in seen_keys:
            return
        seen_keys.add(key)
        evidence.append({
            ...
            "dst_ip": st.dst_ip,
            "dst_port": st.dst_port,
            "packet_rtt_ms": round(rtt_sec * 1000.0, 3),
            "stream_rtt_min_ms": round(st.ack_rtt_min_sec * 1000.0, 3),
            "stream_rtt_max_ms": round(st.ack_rtt_max_sec * 1000.0, 3),
            "stream_rtt_spread_ms": round(max(0.0, st.ack_rtt_max_sec - st.ack_rtt_min_sec) * 1000.0, 3),
            "stream_ack_rtt_samples": st.ack_rtt_samples,
            "stream_high_rtt": "TRUE" if st.high_rtt else "FALSE",
            "stream_spiky_rtt": "TRUE" if st.spiky_rtt else "FALSE",
            "reference_frame_number": acks_frame_no,
            "spiky_baseline_frame_number": st.ack_rtt_min_frame,
            "spiky_baseline_rtt_ms": round(st.ack_rtt_min_sec * 1000.0, 3),
        })
    ...
    for st in rows:
        ...
        if st.high_rtt:
            add_event(
                st, "high_rtt_trigger",
                st.first_high_rtt_frame, st.first_high_rtt_epoch, st.first_high_rtt_rtt_sec,
                st.first_high_rtt_acks_frame,
            )
            add_event(
                st, "high_rtt_max",
                st.ack_rtt_max_frame, st.ack_rtt_max_epoch, st.ack_rtt_max_sec,
                st.ack_rtt_max_acks_frame,
            )
        if st.spiky_rtt:
            add_event(
                st, "spiky_rtt_peak",
    ...
    evidence.sort(key=lambda row: (row["packet_ts"], row["protocol"], row["stream_id"], row["frame_number"], row["event_type"]))
    return evidence


def write_rtt_evidence_csv(out_csv: str, rows: List[StreamAgg], utc: bool) -> None:
    fieldnames = [
        "protocol", "stream_id", "event_type", "frame_number", "packet_ts",
        "src_ip", "src_port", "dst_ip", "dst_port",
        "packet_rtt_ms", "stream_rtt_min_ms", "stream_rtt_max_ms", "stream_rtt_spread_ms",
        "stream_ack_rtt_samples", "stream_high_rtt", "stream_spiky_rtt",
        "reference_frame_number", "spiky_baseline_frame_number", "spiky_baseline_rtt_ms",
    ]
    with open(out_csv, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=fieldnames)
        w.writeheader()
        for row in _rtt_evidence_rows(rows, utc=utc):
            w.writerow(row)
...
def _packet_detail_fields() -> Tuple[List[Tuple[str, str]], Optional[str]]:
    fields: List[Tuple[str, str]] = [
        ...
        ("tcp.ack", "tcp_ack"),
        ("tcp.len", "tcp_len"),
        ("tcp.flags.str", "tcp_flags_str"),
        ("tcp.analysis.ack_rtt", "tcp_analysis_ack_rtt"),
        ("tcp.analysis.acks_frame", "tcp_analysis_acks_frame"),
        ("_ws.col.Protocol", "ws_protocol"),
    ]
    return fields, "_ws.col.info"


def _fetch_packet_details(pcap: str, frame_numbers: List[int]) -> Dict[int, dict]:
    targets = sorted({frame for frame in frame_numbers if frame > 0})
    if not targets:
        return {}

    fields, info_field = _packet_detail_fields()
    frame_filter = " or ".join(f"frame.number=={frame}" for frame in targets)
    cmd = [
        "tshark",
        "-n",
        "-r", pcap,
    ...
    for field_name, _alias in fields:
        cmd.extend(["-e", field_name])
    if info_field:
        cmd.extend(["-e", info_field])
    ...
    aliases = [alias for _field_name, alias in fields]
    if info_field:
        aliases.append("ws_info")
    ...
    for line in proc.stdout.splitlines():
        ...
        if len(parts) < len(aliases):
            parts.extend([""] * (len(aliases) - len(parts)))
        row = dict(zip(aliases, parts))
        try:
            frame_no = int(row.get("frame_number", "0") or "0")
        except ValueError:
            continue
        details[frame_no] = row
    return details


def _format_packet_detail_line(detail: Optional[dict]) -> str:
    if not detail:
        return "Frame detail unavailable."
    src_ip = detail.get("ip_src") or detail.get("ipv6_src") or ""
    dst_ip = detail.get("ip_dst") or detail.get("ipv6_dst") or ""
    protocol = detail.get("ws_protocol") or "TCP"
    frame_no = detail.get("frame_number", "")
    frame_time = detail.get("frame_time", "")
    ack_rtt = detail.get("tcp_analysis_ack_rtt", "")
    time_rel = detail.get("tcp_time_relative", "")
    frame_len = detail.get("frame_len", "")
    stream_id = detail.get("tcp_stream", "")
    info = detail.get("ws_info", "")
    endpoint = f"{src_ip} -> {dst_ip}" if src_ip or dst_ip else ""
    return " ".join(
        part for part in [
            frame_no,
            frame_time,
...
def write_rtt_observation_txt(out_txt: str, pcap: str, rows: List[StreamAgg], utc: bool) -> None:
    evidence_rows = _rtt_evidence_rows(rows, utc=utc)
    frame_numbers: List[int] = []
    for row in evidence_rows:
        frame_numbers.append(int(row.get("frame_number", 0) or 0))
        frame_numbers.append(int(row.get("reference_frame_number", 0) or 0))
    details = _fetch_packet_details(pcap, frame_numbers)

    with open(out_txt, "w", encoding="utf-8") as f:
        for row in evidence_rows:
            obs_frame = int(row.get("frame_number", 0) or 0)
            ref_frame = int(row.get("reference_frame_number", 0) or 0)
            filt = f"frame.number=={obs_frame}" if ref_frame <= 0 else f"frame.number=={obs_frame} or frame.number=={ref_frame}"
            packet_sections = [
                ("Observation Packet", obs_frame, details.get(obs_frame)),
                ("Relative Packet", ref_frame, details.get(ref_frame)),
            ]
            def sort_key(item: tuple) -> tuple:
                _label, frame_no, detail = item
                epoch_s = (detail or {}).get("frame_time_epoch", "")
                try:
                    epoch = float(epoch_s) if epoch_s else float("inf")
                except ValueError:
                    epoch = float("inf")
                return (epoch, frame_no)
            packet_sections.sort(key=sort_key)
            f.write("--------------------------\n")
            f.write(f"Observation Type : {row['event_type']}\n")
            f.write(f"Protocol/Stream  : {row['protocol']} / {row['stream_id']}\n")
            f.write(f"Endpoints        : {row['src_ip']}:{row['src_port']} -> {row['dst_ip']}:{row['dst_port']}\n")
            f.write(f"Observation RTT  : {row['packet_rtt_ms']} ms at frame {obs_frame} ({row['packet_ts']})\n")
            f.write(f"Reference Frame  : {ref_frame}\n")
            if row["event_type"] == "spiky_rtt_peak":
                f.write(
                    f"Spiky Baseline   : frame {row['spiky_baseline_frame_number']} at {row['spiky_baseline_rtt_ms']} ms; "
                    f"spread {row['stream_rtt_spread_ms']} ms\n"
                )
            f.write(f"Filter           : {filt}\n")
            for label, _frame_no, detail in packet_sections:
                f.write(f"{label}:\n")
                f.write(f"{_format_packet_detail_line(detail)}\n")
        if evidence_rows:
            f.write("--------------------------\n")
...
def run_tshark_tls(pcap: str, display_filter: Optional[str]) -> Tuple[List[str], Iterator[str]]:
    ...
    def _run_with_info_field(info_field: str):
        ...
        return tls_fields, _run_tshark_command(cmd)
...
<div id="chipHighRtt" class="chip" data-cat="high-rtt" data-tip="High RTT: stream has ACK RTT at or above 200 ms. See RTT evidence and observation files for the trigger frame.">
High RTT <span id="badgeHighRtt" class="badge">0</span>
</div>
<div id="chipSpikyRtt" class="chip" data-cat="spiky-rtt" data-tip="Spiky RTT: a high-side RTT peak with at least 3 RTT samples and 30 ms or more spread. See RTT evidence and observation files for the peak frame and its related packet.">
  ...
  <div class="panel controls">
    <div class="control-row">
      ...
      <div class="toolbar-actions">
        <button id="showAllGraphs" type="button">Show All Graphs</button>
        <button id="hideAllGraphs" type="button">Hide All Graphs</button>
      </div>
    </div>
...
const hideAllGraphs = document.getElementById('hideAllGraphs');
...
function buildLegend() {
  ...
  updateGraphVisibilityControls();
}

function updateGraphVisibilityControls() {
  const visibleCount = series.filter((s) => s.visible).length;
  showAllGraphs.disabled = visibleCount === series.length;
  hideAllGraphs.disabled = visibleCount === 0;
}
...
function init() {
  ...
  showAllGraphs.addEventListener('click', () => {
    setAllSeriesVisibility(true);
  });

  hideAllGraphs.addEventListener('click', () => {
...
def main():
    ...
    ap.add_argument("--out-rtt-evidence-csv", default=None, help="Output CSV path for RTT evidence frames")
    ap.add_argument("--out-rtt-running-csv", default=None, help="Output CSV path for live per-packet RTT samples")
    ap.add_argument("--out-rtt-observations-txt", default=None, help="Output TXT path for RTT observation details")
    ...
    rtt_evidence_csv_out = unique_name(args.out_rtt_evidence_csv or f"{pcap_base}_rtt_evidence.csv")
    rtt_running_csv_out = unique_name(args.out_rtt_running_csv or f"{pcap_base}_rtt_running.csv")
    rtt_observations_txt_out = unique_name(args.out_rtt_observations_txt or f"{pcap_base}_rtt_observations.txt")

    progress_log(f"Opening RTT running CSV: {rtt_running_csv_out}")
    rtt_running_logger = open_rtt_running_csv(rtt_running_csv_out, utc=args.utc, input_pcap=pcap)
    ...
    ingest_proto(
        ...
        rtt_running_logger=rtt_running_logger,
        utc=args.utc,
    )
    ...
    progress_log(f"Writing RTT evidence CSV: {rtt_evidence_csv_out}")
    write_rtt_evidence_csv(rtt_evidence_csv_out, rows, utc=args.utc)
    progress_log(f"Writing RTT observations TXT: {rtt_observations_txt_out}")
    write_rtt_observation_txt(rtt_observations_txt_out, pcap, rows, utc=args.utc)
    ...
    rtt_running_logger.handle.flush()
    rtt_running_logger.handle.close()
    ...
    print(f"  RTT Evidence CSV: {rtt_evidence_csv_out}")
    print(f"  RTT Running CSV : {rtt_running_csv_out}")
    print(f"  RTT Obs TXT     : {rtt_observations_txt_out}")
