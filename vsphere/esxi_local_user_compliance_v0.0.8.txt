<#
.SYNOPSIS
v0.0.8 of the ESXi local user compliance script.
...
$DesiredEsxAdminsGroupAccessMode = 'accessAdmin'
...
function Show-Usage {
    @'
Usage:
  .\esxi_local_user_compliance_v0.0.8.ps1 -Validate -VMHost esxi01 -EsxAdminsGroup 'DOMAIN\ESX-ADMINS'
  .\esxi_local_user_compliance_v0.0.8.ps1 -Validate -VMHost esxi01 -Username SOCVA -EsxAdminsGroup 'DOMAIN\ESX-ADMINS'
  .\esxi_local_user_compliance_v0.0.8.ps1 -Remediate -VMHost esxi01,esxi02 -EsxAdminsGroup 'DOMAIN\ESX-ADMINS'
  .\esxi_local_user_compliance_v0.0.8.ps1 -Remediate -VMHost esxi01 -Username SOCVA -Password MyPassword! -EsxAdminsGroup 'DOMAIN\ESX-ADMINS'
  .\esxi_local_user_compliance_v0.0.8.ps1 -Remediate -VMHost esxi01 -Username SOCVA -Password MyPassword! -ForceReset -EsxAdminsGroup 'DOMAIN\ESX-ADMINS'
  .\esxi_local_user_compliance_v0.0.8.ps1 -CheckConnectivity -CsvPath .\hosts.csv -Username SOCVA -Password MyPassword! -LockdownMode enable
  .\esxi_local_user_compliance_v0.0.8.ps1 -CheckConnectivity -CsvPath .\hosts.csv -Username SOCVA -Password MyPassword! -ResolutionChunkSize 200
...
Notes:
  ...
  - In remediation mode, the provided ESX admin group is also granted Admin host access on the target ESXi host.
...
function Write-ValidationSnapshotLog {
    ...
    Write-Log -Message "  EsxAdminsGroupAdminAccessExpected: $($Snapshot.EsxAdminsGroupAdminAccessExpected)"
    Write-Log -Message "  EsxAdminsGroupAdminAccessActual: $($Snapshot.EsxAdminsGroupAdminAccessActual)"
    Write-Log -Message "  EsxAdminsGroupAdminAccessStatus: $($Snapshot.EsxAdminsGroupAdminAccessStatus)"
...
function Get-EsxAdminsGroupAdminAccessState {
    param(
        [Parameter(Mandatory = $true)]
        [object]$Context,
    ...
    $result = [ordered]@{
        EsxAdminsGroupAdminAccessExpected = if ($ExpectedGroupName) { $DesiredEsxAdminsGroupAccessMode } else { $null }
        EsxAdminsGroupAdminAccessActual = $null
        EsxAdminsGroupAdminAccessStatus = if ($ExpectedGroupName) { 'Missing' } else { 'Skipped' }
    }

    if (-not $ExpectedGroupName) {
        return [pscustomobject]$result
    }

    $entry = Get-HostGroupAccessEntry -Context $Context -GroupName $ExpectedGroupName
    if (-not $entry) {
        return [pscustomobject]$result
    }

    $actualMode = [string]$entry.AccessMode
    $result.EsxAdminsGroupAdminAccessActual = $actualMode
    $result.EsxAdminsGroupAdminAccessStatus = if ($actualMode -ceq $DesiredEsxAdminsGroupAccessMode) { 'Valid' } else { 'Invalid' }
    return [pscustomobject]$result
}
...
function Get-UserComplianceSnapshot {
    ...
    $snapshot = [ordered]@{
        ...
        EsxAdminsGroupAdminAccessExpected = if ($ExpectedEsxAdminsGroupValue) { $DesiredEsxAdminsGroupAccessMode } else { $null }
        EsxAdminsGroupAdminAccessActual = $null
        EsxAdminsGroupAdminAccessStatus = $null
    }
    ...
    try {
        ...
        $esxAdminsGroupAdminAccessState = Get-EsxAdminsGroupAdminAccessState -Context $Context -ExpectedGroupName $ExpectedEsxAdminsGroupValue
        $snapshot.EsxAdminsGroupAdminAccessExpected = $esxAdminsGroupAdminAccessState.EsxAdminsGroupAdminAccessExpected
        $snapshot.EsxAdminsGroupAdminAccessActual = $esxAdminsGroupAdminAccessState.EsxAdminsGroupAdminAccessActual
        $snapshot.EsxAdminsGroupAdminAccessStatus = $esxAdminsGroupAdminAccessState.EsxAdminsGroupAdminAccessStatus
    }
    catch {
...
function Ensure-EsxAdminsGroupAdminAccess {
    param(
        [Parameter(Mandatory = $true)]
        [object]$Context,
    ...
    if (-not $GroupName) {
        Write-Log -Level 'WARN' -Message "Skipping Admin host access assignment on host '$($Context.VMHost.Name)' because no ESX admin group value is configured."
        return 'Skipped'
    }

    $accessState = Get-EsxAdminsGroupAdminAccessState -Context $Context -ExpectedGroupName $GroupName
    if ($accessState.EsxAdminsGroupAdminAccessStatus -eq 'Valid') {
        Write-Log -Message "Group '$GroupName' already has Admin host access on host '$($Context.VMHost.Name)'."
        return 'AlreadyCompliant'
    }
...
try {
    ...
    if ($cli.Mode -in @('validate', 'remediate')) {
        ...
        if ($effectiveEsxAdminsGroupValue) {
            ...
            Write-Log -Message "Expected host access for group '$effectiveEsxAdminsGroupValue': '$DesiredEsxAdminsGroupAccessMode'."
        }
        else {
            Write-Log -Level 'WARN' -Message "Skipping validation/remediation of advanced setting '$EsxAdminsGroupSettingName' and Admin host access assignment because no desired value is configured in the script and -EsxAdminsGroup was not provided."
        }
    }
    ...
    foreach ($resolvedHost in $resolvedHosts) {
        ...
        if (-not $resolvedHost.IsEligible) {
            ...
            foreach ($currentUsername in $reportUsernames) {
                Add-ReportRow -Row (New-ModeReportRow -Mode $cli.Mode -Data ([ordered]@{
                    ...
                    EsxAdminsGroupAdminAccessExpected = if ($effectiveEsxAdminsGroupValue) { $DesiredEsxAdminsGroupAccessMode } else { $null }
                    EsxAdminsGroupAdminAccessActual = $null
                    EsxAdminsGroupAdminAccessStatus = 'Skipped'
        ...
        foreach ($currentUsername in $reportUsernames) {
            ...
            $esxAdminsGroupAdminAccessRemediationStatus = if ($cli.Mode -eq 'remediate') {
                if ($effectiveEsxAdminsGroupValue) { 'NotRequested' } else { 'Skipped' }
            } else { 'NotApplicable' }
            ...
            try {
                if ($cli.Mode -eq 'remediate') {
                    ...
                    if ($effectiveEsxAdminsGroupValue) {
                        ...
                        if ($snapshot.EsxAdminsGroupAdminAccessStatus -ne 'Valid') {
                            ...
                            Write-Log -Message "Group '$effectiveEsxAdminsGroupValue' already has Admin host access on host '$($context.VMHost.Name)'."
                            $esxAdminsGroupAdminAccessRemediationStatus = 'AlreadyCompliant'
                        }
                    }
                ...
                if ($cli.Mode -eq 'remediate' -and $esxAdminsGroupAdminAccessRemediationStatus -eq 'NotRequested') {
                    $esxAdminsGroupAdminAccessRemediationStatus = 'Failed'
                }
            ...
            Add-ReportRow -Row (New-ModeReportRow -Mode $cli.Mode -Data ([ordered]@{
                ...
                EsxAdminsGroupAdminAccessExpected = $snapshot.EsxAdminsGroupAdminAccessExpected
                EsxAdminsGroupAdminAccessActual = $snapshot.EsxAdminsGroupAdminAccessActual
                EsxAdminsGroupAdminAccessStatus = $snapshot.EsxAdminsGroupAdminAccessStatus
                EsxAdminsGroupRemediationStatus = $esxAdminsGroupRemediationStatus
                EsxAdminsGroupAdminAccessRemediationStatus = $esxAdminsGroupAdminAccessRemediationStatus
