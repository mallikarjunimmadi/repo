...
# Outputs are auto-prefixed with the NSX host (FQDN/IP):
...
def list_services(nsx: NSXClient) -> Dict[str, dict]:
    ...
    for s in svcs:
        ...
        if path:
            out[path] = s
    logging.info("Discovered %d Services", len(out))
    return out
...
def list_groups_for_domain(nsx: NSXClient, domain_id: str) -> Dict[str, dict]:
    ...
    for g in groups:
        ...
        if path:
            m[path] = g
    logging.info("Domain '%s': %d Groups", domain_id, len(m))
    return m
...
def build_reports(nsx: NSXClient, out_prefix: str):
    services_by_path = list_services(nsx)
    ...
    for d in domains:
        ...
        if not did:
            continue
        groups_by_path.update(list_groups_for_domain(nsx, did))
    logging.info("Total Groups across all domains: %d", len(groups_by_path))
...
def main():
    ...
    logging.info("Starting where-used scan: nsx_host=%s verify_ssl=%s prefix=%s", nsx_host, verify_ssl, out_prefix)
    ...
    try:
        s_usage, s_unused, g_usage, g_unused = build_reports(nsx, out_prefix)
