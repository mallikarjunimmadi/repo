...
def finalize_tcp_flags(rows: List[StreamAgg]) -> None:
    """
    Make the 3 TCP failure types mutually exclusive (no overlap).

    Definitions (TCP only):
      conn_refused_syn_rst:
        - SYN observed
        - RST observed
        - no SYN-ACK
        - no other TCP flag types beyond SYN/RST (as tracked by other_flag_pkts)

      conn_failed_ackretrans_rst:
        - SYN observed
        - SYN-ACK observed AND retransmitted (inferred as synack_pkts >= 2)
        - RST may or may not be present
        - no other TCP flag types beyond SYN/SYN-ACK (as tracked by other_flag_pkts)

      client_abort_after_synack:
        - SYN observed
        - SYN-ACK observed
        - client aborts/tears down early (RST with no ACK counted in rst_no_ack_pkts)
        - no other TCP flag types beyond the tracked handshake/abort flags
    ...
    for st in rows:
        ...
        # Connection Failed: SYN + SYN-ACK retransmits (RST optional)
        conn_failed = (
            st.syn_pkts >= 1 and
        ...
        # Mutual exclusivity by priority
        if conn_refused:
...
<head>
  ...
  <style>
    ...
    .row-refused {{ background: rgba(255, 99, 71, 0.10); }}   /* tomato-ish */
    .row-abort   {{ background: rgba(255, 215, 0, 0.12); }}   /* gold-ish */
    .row-failed  {{ background: rgba(30, 144, 255, 0.10); }}  /* dodger-ish */
}}
    .chip.on {{ 
      border-color: #2e7d32; 
      background: #e8f5e9; 
    ...
    .grid {{
      display: grid;
      grid-template-columns: 70px 70px 220px 90px 220px 90px 95px 95px 230px 230px 120px 90px 110px 70px 70px 70px 85px 170px 190px 170px;
      min-width: 2100px;
      column-gap: 10px;
      align-items: center;
      padding: 0 10px;
...
<body>
  ...
  <div class="bar">
    ...
    <div id="chipConnRefused" class="chip" title="Server actively refused: SYN followed by RST (no SYN-ACK).">
      Connection refused <span class="badge" id="badgeConnRefused">0</span>
    </div>
    <div id="chipClientAbort" class="chip" title="Client abort: handshake started (SYN/SYN-ACK) then client resets early.">
      Client abort <span class="badge" id="badgeClientAbort">0</span>
    </div>
    <div id="chipConnFailed" class="chip" title="Connection failed: SYN then SYN-ACK retransmits (RST optional), handshake not completed.">
      Connection failed <span class="badge" id="badgeConnFailed">0</span>
    </div>
  </div>
...
(() => {{
  ...
  // Chip count badges (based on full dataset)
  ...
  const cntRefused = data.reduce((n, r) => n + (r.protocol === 'TCP' && r.conn_refused_syn_rst ? 1 : 0), 0);
  const cntAbort   = data.reduce((n, r) => n + (r.protocol === 'TCP' && r.client_abort_after_synack ? 1 : 0), 0);
  const cntFailed  = data.reduce((n, r) => n + (r.protocol === 'TCP' && r.conn_failed_ackretrans_rst ? 1 : 0), 0);

  if (badgeConnRefused) badgeConnRefused.textContent = String(cntRefused);
  if (badgeClientAbort) badgeClientAbort.textContent = String(cntAbort);
  if (badgeConnFailed)  badgeConnFailed.textContent  = String(cntFailed);
...
function renderPairs() {{
  ...
  function makeRow(r) {{
    ...
    // Row color coding (mutually exclusive by backend, but keep safe fallback)
    let cls = 'grid';
    if (r.conn_refused_syn_rst) cls += ' row-refused';
    else if (r.conn_failed_ackretrans_rst) cls += ' row-failed';
    else if (r.client_abort_after_synack) cls += ' row-abort';
    div.className = cls;
